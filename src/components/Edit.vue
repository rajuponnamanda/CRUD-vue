<template>
  <div class="shadow-md pb-6">
    <div>
      <h3 class="text-center text-dark  pt-2 pb-2">Edit Student</h3>
    </div>
    ...
    <div class="text-center">
      ...
      <router-link to="/"
        class="btn btn-secondary text-white font-medium py-2 my-5 px-5 mx-5">
        Back
      </router-link>
...
function updateDetails() {
  axios
    .put(`http://localhost:3000/users/${id}`, studentData.value ,id)
    .then((res) => {
      studentData.value = res.data;
      router.push('/')
      {
